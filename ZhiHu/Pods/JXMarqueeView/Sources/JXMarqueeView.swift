...
public class JXMarqueeView: UIView {
    ...
    public var contentViewFrameConfigWhenCantMarquee: ((UIView) -> Void)? // 当contentView的内容宽度没有超过显示宽度，无需开启跑马灯效果。这个时候contentView的size，默认是调用sizeToFit之后的尺寸。如果想要特殊配置，比如让contentView的size等于JXMarqueeView，就需要在该闭包自定义配置。
    ...
    open override func layoutSubviews() {
        ...
        if marqueeType == .reverse {
            containerView.frame = CGRect(x: 0, y: 0, width: validContentView.bounds.size.width, height: bounds.size.height)
        } else {
            containerView.frame = CGRect(x: 0, y: 0, width: validContentView.bounds.size.width * 2 + contentMargin, height: bounds.size.height)
        }

        if validContentView.bounds.size.width > bounds.size.width {
            validContentView.frame = CGRect(x: 0, y: 0, width: validContentView.bounds.size.width, height: bounds.size.height)
            if marqueeType != .reverse {
                // 骚操作：UIView是没有遵从拷贝协议的。可以通过UIView支持NSCoding协议，间接来复制一个视图
                let otherContentView = validContentView.copyMarqueeView()
                otherContentView.frame = CGRect(x: validContentView.bounds.size.width + contentMargin, y: 0, width: validContentView.bounds.size.width, height: bounds.size.height)
    ...
    fileprivate func startMarquee() {
        ...
        marqueeDisplayLink = CADisplayLink(target: self, selector: #selector(processMarquee))
        marqueeDisplayLink?.frameInterval = frameInterval
    ...
    @objc fileprivate func processMarquee() {
        ...
        case .right:
            let targetX = bounds.size.width - contentView!.bounds.size.width
            if frame.origin.x >= targetX {
                frame.origin.x = bounds.size.width - containerView.bounds.size.width
                containerView.frame = frame
            } else {
                frame.origin.x += pointsPerFrame
                if frame.origin.x > targetX {
        ...
        case .reverse:
            if isReversing {
                ...
                let targetX = bounds.size.width - containerView.bounds.size.width
                if frame.origin.x <= targetX {
                    isReversing = true
                } else {
                    frame.origin.x -= pointsPerFrame
